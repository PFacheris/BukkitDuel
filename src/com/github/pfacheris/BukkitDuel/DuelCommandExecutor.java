...
public class DuelCommandExecutor
implements CommandExecutor
{
	private BukkitDuel plugin;

	public DuelCommandExecutor(BukkitDuel plugin)
	{
		this.plugin = plugin;
	...
	private boolean performChallengeCommand(CommandSender sender, String[] args)
	{
		if (args.length == 1)
		{
		...
		if (args.length > 1)
		{
			Player challengee = plugin.getServer().getPlayer(args[1]);
			if (challengee != null && challengee.isOnline())
			{
				...
				if(args.length == 2)
				{
					if (BukkitDuel.duelManager.getDuelByTwoParticipants((Player)sender, challengee) == null)
					{
						...
						if (tempArena != null)
						{
							Duel tempDuel = new Duel((Player)sender, challengee, tempArena);
				...
				else if(args.length == 3)
				{
					...
					{
						Arena tempArena = BukkitDuel.arenaManager.getArenaByName(args[2]);
						if (tempArena != null)
						{
	...
	private boolean performAcceptCommand(CommandSender sender, String[] args)
	{
		if (args.length == 1)
		{
		...
		if (args.length == 2)
		{
			Player initiator = plugin.getServer().getPlayer(args[1]);
	...
	private boolean performRejectCommand(CommandSender sender, String[] args)
	{
		if (args.length == 1)
		{
			if (BukkitDuel.duelManager.getDuelByChallengee((Player) sender).size() > 1)
			...
			if (BukkitDuel.duelManager.getDuelByChallengee((Player) sender).size() == 1)
			{
				Duel tempDuel = BukkitDuel.duelManager.getDuelByChallengee((Player) sender).get(0);
		...
		if (args.length == 2)
		{
			...
			if (initiator != null)
			{
				if (BukkitDuel.duelManager.getDuelByTwoParticipants(initiator, (Player)sender) != null)
				{
					Duel tempDuel = BukkitDuel.duelManager.getDuelByTwoParticipants(initiator, (Player)sender);
					BukkitDuel.duelManager.cancelDuel(tempDuel);
					sender.sendMessage("[BukkitDuel] Request from " + tempDuel.getInitiator().getName() + " rejected.");
					tempDuel.getInitiator().sendMessage("[BukkitDuel] Request to " + sender.getName() + " rejected.");
					return true;
	...
	private boolean performRequestCommand(CommandSender sender, String[] args)
	{	
		sender.sendMessage(ChatColor.GREEN + "===========" + ChatColor.BOLD + "[Requests To Me]" + ChatColor.GREEN + "===========");
		int counter = 0;
		for(Duel d : BukkitDuel.duelManager.getDuelByChallengee((Player)sender))
		{
		...
		for(Duel d : BukkitDuel.duelManager.getDuelByInitiator((Player)sender))
		{
	...
	private boolean performArenaCommand(CommandSender sender, String[] args)
	{
		if (args.length == 1)
		{
			sender.sendMessage(ChatColor.GREEN + "============" + ChatColor.BOLD + "[Vacant Arenas]" + ChatColor.GREEN + "===========");
			for(Arena a : BukkitDuel.arenaManager.getArenasByUsed(false))
			{
			...
			for(Arena a : BukkitDuel.arenaManager.getArenasByUsed(true))
			{
				sender.sendMessage(ChatColor.BOLD + "Arena Name: " + ChatColor.GREEN + a.getName());
			}
		...
		if (args.length > 1)
		{
			if (args[1].equalsIgnoreCase("set"))
			{
				if (BukkitDuel.duelManager.getDuelByInitiator((Player)sender).size() == 1)
				{
					if (args.length > 2)
					{
						Arena newArena = BukkitDuel.arenaManager.getArenaByName(args[2]);
						if (newArena != null)
						{
				...
				if (BukkitDuel.duelManager.getDuelByInitiator((Player)sender).size() > 1)
				{
					if (args[3] != null)
					{
						...
						if (newArena != null)
						{
							Duel tempDuel = BukkitDuel.duelManager.getDuelByTwoParticipants((Player)sender,plugin.getServer().getPlayer(args[3]));
							if (tempDuel != null)
							{
			...
			if (args[1].equalsIgnoreCase("add"))
			{
				if (((Player)sender).hasPermission("BukkitDuel.admin"))
				{
					...
					if (sel instanceof CuboidSelection) {
						CuboidSelection polySel = (CuboidSelection) sel;
						int minY = polySel.getNativeMinimumPoint().getBlockY();
						int maxY = polySel.getNativeMaximumPoint().getBlockY();
						int minX = polySel.getNativeMinimumPoint().getBlockX();
						int maxX = polySel.getNativeMaximumPoint().getBlockX();
						int minZ = polySel.getNativeMinimumPoint().getBlockZ();
						int maxZ = polySel.getNativeMaximumPoint().getBlockZ();
		...
		if (args.length >= 2)
		{
			...
			if (playerName.equalsIgnoreCase("view"))
			{
				if (args.length >= 3)
				{
					...
					Duel tempDuel = BukkitDuel.duelManager.getDuelByTwoParticipants((Player)sender,plugin.getServer().getPlayer(playerName));
					...
					if (tempDuel.getInitiator().equals((Player)sender))
					{
						sender.sendMessage("Items: Can be modified and viewed using '/duel stake <opponent> item'");
					}
					else
					{
						for(ItemStack item : tempDuel.stakesInitiator.getWagerItems().getContents())
						{
					...
					if (tempDuel.getChallengee().equals((Player)sender))
					{
						sender.sendMessage("Items: Can be modified and viewed using '/duel stake <opponent> item'");
					}
					else
			...
			if (args.length >= 3)
			{
				...
				if (id.equalsIgnoreCase("item"))
				{
					...
					if (tempDuel.getInitiator().equals((Player)sender))
					{
						...
						tempDuel.getChallengee().sendMessage("[BukkitDuel] " + sender.getName() + " has altered their wager items.\nUse '/duel stake view <player>' to check the wager.");

					}
					if (tempDuel.getChallengee().equals((Player)sender))
					{
						((Player)sender).openInventory(tempDuel.stakesChallengee.getWagerItems());
						tempDuel.getInitiator().sendMessage("[BukkitDuel] " + sender.getName() + " has altered their wager items.\nUse '/duel stake view <player>' to check the wager.");
					}
					return true;
				}
				if (args.length >= 4)
	...
	private boolean performHelpCommand(CommandSender sender, String[] args)
	{
